...
그래서 우회했다. Nanobanana에서 먼저 캐릭터의 바스트샷(가슴 위로 잘린 상반신 사진) 사이즈 이미지를 뽑았다. 여기서 원하는 얼굴과 분위기가 나올 때까지 조정한 뒤, 멀티샷 기능을 활용해 같은 인물의 정면, 45도, 측면 등 다양한 각도의 이미지를 만들었다.
...
캐릭터 이미지와 로케이션 이미지가 준비되면, 이것들을 Higgsfield Cinema Studio로 가져와서 씬 이미지를 만들었다. 이 사람이 이 공간에 서 있는 한 장의 사진을. 그리고 이 씬을 바탕으로 여러 멀티샷들을 만들었다. 필요한 경우 3D 씬에서 특정 각도의 컷을 캡쳐해 생성했다.
...
이렇게 초 단위로 동작의 시작과 끝을 지정하니, 두 가지가 동시에 해결됐다. 첫째, AI가 빈 시간을 엉뚱한 것으로 채우는 문제가 사라졌다. 둘째, 동작의 앞뒤로 자연스러운 여유가 생겼다. 실제 연기에서 배우가 동작 전후에 보여주는 미세한 호흡 — 손을 올리기 전 잠깐의 망설임, 내린 뒤의 고요함 — 이런 것들이 시간을 지정해줌으로써 오히려 살아났다.

이것이 이 프로젝트에서 얻은 가장 큰 발견이었다. AI 영상에서 '연출'이란, 무엇을 하라고 지시하는 것뿐 아니라 시간을 설계하는 것이라는 점. 트리밍이 가능한 여유가 생겼다! (하지만 2017년산 아이맥이 프리미어에서 트림을 할 때마다 그린 스크린을 보여줬다...)


— 편집: OK컷과 Fine컷

이렇게 생성한 영상들 중에서 OK컷과 Fine컷을 골라 프리미어 프로에서 편집했다.

사운드 디자인도 손을 보고 싶었다. 발소리, 문 여닫는 소리, 공간의 앰비언스. 하지만 예산 문제로 이 부분은 타협했다. 모든 것을 한 번에 완성할 수는 없었다.


— 돌아보면

이 프로젝트의 의미는 결과물 자체보다, 과정에 있다.
...
영화를 전통적으로 만들 때는 기록이 자연스럽게 쌓인다. 촬영 시트, 스크립트, 편집 로그 등등 — 시스템 자체가 기록을 만들어내는 구조다. 하지만 AI 영상 제작은 달랐다. 혼자서 프롬프트를 쓰고, 클립을 생성하고, 마음에 안 들면 다시 돌리고 — 이 과정이 어디에도 남지 않았다.
